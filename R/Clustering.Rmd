...
variables <- list() #list("ASSQ_Total", "SRS_Total")

fsGLMdata <- asdZScored[asdZScored$site == fsgdSite,][,c("lh.aparc.thickness", "ASSQ_Total","SRS_Total", "clust")]
fsGLMdata$clust = 2
hcSubs <- data[data$site == fsgdSite & data$Subject == 0,][,c("lh.aparc.thickness", "ASSQ_Total","SRS_Total")]
hcSubs$clust <- 1
...
fsgdFile = paste("ASDanatomy_all_",fsgdSite,".fsgd",sep = "")
f <- generateFSGD(title, list(class1,class2), variables, fsGLMdata, fsgdFile)

source("GLM_helper.R")
cmd <- generateGroupComparisonCommands(
  subjectDir = "/mnt/exchange/Site-SI_Derivatives_UZH",
  analysis = "DEMO",
  hemi = "lh",
  fsgdFile = fsgdFile,
