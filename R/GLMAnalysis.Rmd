...
```{r}
project <- params$GLMProjects$ClusteredASDvHC2$Title
analysisNames <- c()
glmResultFiles <- list()
glmResultData <- data.frame()
for (hemi in params$Hemis){
  for (feature in params$Features){
    for (contrast in params$GLMProjects$ClusteredASDvHC2$ContrastPerms) {
      dir <- getResultsDirPath(baseDir = params$GLMFolder, hemisphere = hemi, 
                               projectname = project, feature =  feature,
...
glmOverview
write.csv(glmOverview, file="glmOverview.csv", sep=";")
```
```{r}
...
displayMGH(glmResultFiles$ClusterASDvHCComparison$noVars,"thickness", "cluster")

displayMGH(glmResultFiles$ClusterASDvHCComparison$noVars,"area", "cluster")
displayMGH(glmResultFiles$ClusterASDvHCComparison$Vars,"thickness", "cluster")
displayMGH(glmResultFiles$ClusterASDvHCComparison$Vars,"area", "cluster")

displayMGH(glmResultFiles$ClusterASDvHCComparison$`1-10`,"area", "cluster")
displayMGH(glmResultFiles$ClusterASDvHCComparison$`1-10`,"thickness", "cluster")
