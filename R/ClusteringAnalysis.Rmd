...
post_hoc_analysis <- function(data, measure, grouping = "clust", adjustP = T) {
  density_plot <- ggplot(data, aes(x= data[,measure],  color = as.factor(data[,grouping]), group = data[,grouping])) + geom_density() + theme_apa() + ylab("")+xlab(measure) + theme(text=element_text(family="serif"))
  box_plot <- ggplot(data, aes(y= as.numeric(data[,measure]),  color = as.factor(data[,grouping]), group = data[,grouping])) + geom_boxplot() +theme_apa() +ylab(measure) + theme(text=element_text(family="serif"))
...
# ASDMeasures$p <- Nyholt.Bonferroni.p(asdZScored[,asdScales], ASDMeasures$p)
ASDMeasures
