...
srs_ByASD <- ggplot(data, aes(x = SRS_Total,group = Subject, fill = factor(Subject))) +
  geom_histogram( position = "identity", bins = 15, alpha = 0.70) + 
  labs(fill = "SRS" ) + theme_apa() +
...
assq_ByASD <- ggplot(data, aes(x = ASSQ_Total,group = Subject, fill = factor(Subject))) +
  geom_histogram( position = "identity", bins = 15, alpha = 0.70) + 
  labs(fill = "SRS" ) + theme_apa() + 
  theme(axis.text=element_text(size=12), axis.title=element_text(size=12,face="bold"), legend.position = c(0.9,0.87)) +
